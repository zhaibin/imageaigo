...
**背景**: 发现 471 张图片中，只有 2 张生成了 WebP 展示版本，其余 469 张是旧数据

**问题影响**:
- ❌ 57 张大图片（> 1080px）没有展示版本
- ❌ 前端加载慢（原图 2-5MB）
- ❌ 浪费带宽 40-60%
- ❌ 用户体验不一致
...
| 指标 | 迁移前 | 迁移后 | 改进 |
|------|--------|--------|------|
| 有展示版本的图片 | 2 张 | 59 张 | ⬆️ +57 张 |
| 大图加载速度 | 2-5MB 原图 | 150-300KB WebP | ⬇️ 85-95% |
| 带宽消耗 | 高 | 低 | ⬇️ 40-60% |
| Image Resizing 调用 | 8 次 | 65 次 | ⬆️ +57 次 |

#### 部署说明

部署后，进入管理后台 > 系统管理 > 图片迁移，点击按钮开始迁移。
...
**问题**: 批量上传和 Unsplash 同步改为串行处理后，前端进度对话框不显示
...
- ✅ 添加批次状态初始化（之前被移除）
- ✅ 每张图片处理时实时更新状态
- ✅ 支持 `processing`, `completed`, `skipped`, `failed` 状态
- ✅ 完成后自动标记批次为完成
- ✅ 前端右上角实时显示进度
...
```javascript
{
  batchId: "batch_xxx" | "unsplash_xxx",
  total: 30,
  completed: 5,
  skipped: 2,
  failed: 1,
...
**更新时机**:
1. 批次初始化：立即写入 KV
2. 预处理开始：更新 lastActivity
3. 每个文件处理：更新文件状态
4. 批次完成：标记 status = 'completed'
...
用户上传批次或触发 Unsplash 同步后，右上角会立即显示进度面板：

```
⚙️ 处理任务 [2]

📤 批次 #1730000000 (20 张)          [取消]
━━━━━━━━━━━━━━━━━━━ 65% ━━━━━━━━━━━━━
✅ 完成: 10  ⏭️ 跳过: 3  ❌ 失败: 0  ⏳ 处理中: 1
正在处理: image-05.jpg
用时: 1:25

🌐 Unsplash #1730000100 (30 张)     [取消]
━━━━━━━━━━━━━━━━━━━ 40% ━━━━━━━━━━━━━
✅ 完成: 8  ⏭️ 跳过: 4  ❌ 失败: 0  ⏳ 处理中: 1
正在处理: unsplash-abc123.jpg
用时: 2:10
```
